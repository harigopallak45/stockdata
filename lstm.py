...
# LSTM Prediction for next 3 months (90 days)
def lstm_prediction(filepath):
    try:
        ...
        # Predict the next 90 days (3 months)
        future_predictions = []
        ...
        future_dates = [last_date + pd.Timedelta(days=i) for i in range(1, 91)]  # Next 90 days
        ...
        ax.plot(future_dates, future_predictions, label='Future Predicted Price (Next 3 Months)', color='green', linestyle='dashed')
        ax.set_xlabel('Date')
        ax.set_ylabel('Close Price')
        ax.set_title('Improved LSTM Prediction with Future Forecast (Next 3 Months)')
        ax.legend()
        ax.grid(True)

        # Save the plot
        plot_filename = os.path.join(uploads_folder, 'lstm_prediction_future_3_months_improved.png')
...
# Function to run the LSTM prediction
